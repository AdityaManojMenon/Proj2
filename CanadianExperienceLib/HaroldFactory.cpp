...
std::shared_ptr<Actor> HaroldFactory::Create(std::wstring imagesDir)
{
    ...
    rarm->AddPoint(wxPoint(-25, 96));  // Angled outward at bottom
    rarm->AddPoint(wxPoint(-10, 96));  // Adjusted width
    ...
    rhand->SetPosition(wxPoint(-17, 96));  // Adjusted to match arm endpoint
    rhand->AddPoint(wxPoint(-5, -2));
    rhand->AddPoint(wxPoint(-15, 12));
    rhand->AddPoint(wxPoint(8, 22));
    rhand->AddPoint(wxPoint(18, 8));
    rarm->AddChild(rhand);

    // Add chainsaw to Harold's right hand
    auto chainsaw = std::make_shared<ImageDrawable>(L"Chainsaw", imagesDir + L"/chainsaw.png");
    chainsaw->SetCenter(wxPoint(20, 170));
    chainsaw->SetPosition(wxPoint(-15, -30));
    rhand->AddChild(chainsaw);
